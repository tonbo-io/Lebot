...
def get_active_issues_by_assignee(linear: LinearClient, team_id: Optional[str] = None) -> Dict[str, List[Dict[str, Any]]]:
    ...
    if "history" in issue and issue["history"]["nodes"]:
        for history_item in issue["history"]["nodes"]:
            if history_item.get("fromState") and history_item.get("toState"):
                change_date = datetime.fromisoformat(history_item["createdAt"].replace("Z", "+00:00")).replace(tzinfo=None)
...
def main():
    ...
    parser.add_argument("--days", type=int, default=3, help="Number of days to check for complete inactivity (default: 3)")
        ...
        for issue in issues:
            ...
            if last_activity:
                issue_activities.append(
                    {"issue": issue, "last_activity": last_activity, "days_inactive": (datetime.now() - last_activity).days}
                )
